...
S08 严哨/红牌/身体对抗混沌：允许 胜/负、负/胜 等反转；VAR/点球见 S16
S11 小组积分控节奏：偏 平/平、平/胜、平/负
S13 必须抢分开局冒进：偏 胜/胜、负/负、平/胜、平/负
S16 VAR/点球节奏改变：偏 胜/负、负/胜、平/胜、平/负
```

### V3.2 校准规则（S11–S16）

EventFlow V3.2 使用 **deterministic eventflow inference**（确定性事件流推演），不是 Monte Carlo 随机模拟。每场只跑一条权重分解链，输出可解释的 `weight_composition`。
