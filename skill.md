...
- 取消 85% 固定胜率上限：证据充分且校准结果支持时，胜率可以超过 85%
- 任何胜/平/负概率都不得等于 100% 或 0%；数值实现使用极小 ε 做开区间保护，并保留非零爆冷/意外路径
- 高概率且通过数据覆盖、时间外校准、模型可靠度、概率派/EventFlow一致性及赔率有效性检查的选项，可以标记为“稳胆”；“稳胆”表示本批候选中的相对优先级，不表示必胜、保本或无风险。极端概率（如 >85%）仍必须同时报告主要反向情景
...
5. **半场胜平负 / 半全场胜负**（从主队视角）：可输出排序与概率，但默认标记为**低置信度层**。半场样本更短、单个进球和比赛状态冲击更大，且当前两段泊松与 EventFlow 半场分布尚未完成充分校准；不得与全场胜平负概率同等级解读
...
- “稳胆”仅用于全场层的高概率候选，例如全场胜平负、让球方向、总进球区间或其他已经完成历史校准的玩法。
- 标记“稳胆”前须同时满足：数据覆盖充分、最终置信度为高、概率派与 EventFlow 不存在重大方向冲突、赔率/盘口有效且没有明显负价值、无未解决的关键伤停或首发冲突。
- “稳胆”是相对标签，可用于主仓或串关胆项，但必须同时列出失败路径和建议仓位；不得等同于“必胜”“稳赚”“保本”。
- 半场胜平负、半全场当前默认低置信度，不得标记为“稳胆”，也不得作为主仓或串关唯一胆项。

### 半场 / 半全场低置信度纪律

- 当前半场胜平负和半全场分布主要来自上下半场两段泊松、比赛状态假设与 EventFlow 排序，尚未完成与全场胜平负同等级的历史校准。
- 默认 `confidence = low`，即使某一半全场组合排名第一，也只表示相对排序，不表示高命中率。
- 预测报告必须把半场/半全场放在独立的“低置信度参考”区，并说明最敏感变量：早球、点球、红牌、临场换人、伤退、天气中断等。
- 投注策略中，HAFU 不得进入稳健层或主仓；只能作为高风险小注，且必须同时满足：玩法已开售、SP有效、多源证据一致、无关键阵容缺口、历史时间外校准通过。
- 在未完成至少一个完整赛事阶段的时间外验证前，不得根据少量命中或失误反向调整半场权重。
...
建议包含：
- 比赛结论：必须包含倾向、预测比分、胜/平/负概率、预测比分分布及概率、预期进球及概率、大小球概率分布
- 半场胜平负 / 半全场可以补充，但须单列“低置信度参考”，展示分布而非单点断言；数据或校准不足时允许省略
...
5. 若概率派与 EventFlow 分歧，优先用融合结果决定玩法类型（总进球 / 比分包等）。半场胜平负 / 半全场因当前校准不足，不得仅凭 EventFlow 排名进入核心推荐。
...
- 策略方向：胜平负、让球、总进球、比分等（玩法定义见规则文件）
- 半场胜平负 / 半全场：默认仅列入高风险候选，标注“低置信度、小注、非核心”；不得作为稳健层、主仓、单关保底或串关唯一胆项。只有在独立时间外回测达到预设校准门槛后，才允许提升等级
...
| **L2** | **动态反击/定位球地板** | 弱队 λ 使用由反击质量、强弱差与数据置信度共同约束的动态下限；不再使用固定 `0.85 × 反击质量`。该层用于防止世界杯级球队被层层修正压成近零，但不得系统性抬高 BTTS/Over。 |
...
| **L9** | **Dixon-Coles + 两段式** | 低比分 τ 相关性修正（ρ=−0.045）；并用**上下半场两段泊松**建模——崩溃倍率把强队进球更多压到下半场，用于生成**待校准的半场/半全场参考分布**（如平/胜、胜/胜）；该层当前默认低置信度，不得写成“真实半全场概率”。 |
